...
async def create_start_pose(client: EventClient | None = None, timeout: float = 0.5) -> Pose3F64:
    ...
    json_pose0 = "/home/natalr/Documents/xdipper/data/tracks/pose0.json"
    pdb.set_trace()
    track0 = proto_from_json_file(json_pose0, Track())

    pose0 = track0.waypoints[0]
    pdb.set_trace()
    zero_tangent = np.zeros((6, 1), dtype=np.float64)
    translation = np.asarray([pose0.a_from_b.translation.x, pose0.a_from_b.translation.y, 0])
    rotation = Rotation3F64.Rz(45)
    a_from_b = Isometry3F64(translation=translation, rotation=rotation)
    start: Pose3F64 = Pose3F64(
        a_from_b=a_from_b, frame_a="world", frame_b="robot", tangent_of_b_in_a=zero_tangent
    )
    # zero_tangent = np.zeros((6, 1), dtype=np.float64)
    # start: Pose3F64 = Pose3F64(
...
async def build_track(reverse: bool, client: EventClient | None = None, save_track: Path | None = None) -> Track:
    ...
    row_spacing: float = 6  # 48 inches in meters
    row_length: float = 24  # 32 feet in meters
    ...
    track_builder.create_straight_segment(next_frame_b="goal1", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal2", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal3", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal4", distance=row_spacing, spacing=0.1)
    # track_builder.create_straight_segment(next_frame_b="goal5", distance=row_spacing, spacing=0.5)
    # track_builder.create_straight_segment(next_frame_b="goal6", distance=row_spacing, spacing=0.5)
    track_builder.create_arc_segment(next_frame_b="goal5", radius=row_spacing/2, angle=radians(180), spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal6", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal7", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal8", distance=row_spacing, spacing=0.1)
    track_builder.create_straight_segment(next_frame_b="goal9", distance=row_spacing, spacing=0.1)
    # track_builder.create_straight_segment(next_frame_b="goal10", distance=row_spacing, spacing=0.5)
    # track_builder.create_straight_segment(next_frame_b="goal13", distance=row_spacing, spacing=0.5)
    # track_builder.create_arc_segment(next_frame_b="goal14", radius=row_spacing, angle=radians(180), spacing=0.1)
    ...
    if save_track is not None:
        ...
        import pdb
        pdb.set_trace()
        if not proto_to_json_file("/home/natalr/Documents/xdipper/data/tracks/planned_11262024_v1.json", track_builder.track):
            raise RuntimeError(f"Failed to write Track")
