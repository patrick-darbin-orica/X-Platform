...
async def main(service_config_path: Path, track_path: Path) -> None:
    ...
    move_next = True
    if move_next:
        await EventClient(service_config).request_reply("/clear_track", Empty())
        track: Track = proto_from_json_file(track_path, Track()) # Track with only the next waypoint
        # Send the track to the track_follower
        await set_track(service_config, track)
        # Follow the track
        await start(service_config)
        move_next = input("Move to the next point?")
        move_next = move_next in ["y","Y"]
