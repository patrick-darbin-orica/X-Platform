...
Usage:
    python3 encode_primer.py --hole 12 --index 1
    python3 encode_primer.py --hole 12 --index 1 --dry-run
    python3 encode_primer.py --host 10.95.76.24 --hole 12 --index 1

Arguments:
    --host      Windows bridge IP address (default: 10.95.76.24)
...
def main():
    ...
    parser.add_argument(
        "--index", type=int, required=True, help="Primer number, 1-based"
    )
    ...
    with AmigaBridgeClient(host=args.host, port=args.port) as client:
        ...
        if drx_index >= len(primers):
            print(
                f"ERROR: Hole '{hole_name}' has {len(primers)} primer(s). "
        ...
        if not primer_uid:
            print(
                f"ERROR: primerUID is empty for hole {hole_name}, index {args.index}."
            )
            sys.exit(1)
        ...
        if verify_holes:
            ...
            if verify_primers and drx_index < len(verify_primers):
                ...
                print(f"Encoded: {is_encoded}  |  detStatus: '{det_status}'  |  DRXId: '{drx_id}'")
